...
void Estimator::optimization()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
     ...
     TicToc t_cov;
     marginalization_info->latest_pose_addr =reinterpret_cast<long>(para_Pose[WINDOW_SIZE]);
     if (optimizationDegeneracyDetection(problem, 50.0, marginalization_info))
     {
        printf("Detected degeneracy in optimization!");
     }

    ROS_INFO("Optimization degeneracy costs: %f", t_cov.toc());
...
bool Estimator::optimizationDegeneracyDetection(ceres::Problem &problem, const double eigen_thresh, MarginalizationInfo *marginalization_info)
{
    ...
    cov_blocks.push_back(std::make_pair(para_Pose[WINDOW_SIZE - 1], para_Pose[WINDOW_SIZE - 1]));
    ...
    covariance.GetCovarianceBlockInTangentSpace(para_Pose[WINDOW_SIZE-1], para_Pose[WINDOW_SIZE-1], cov_pose.data());

    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double, 6, 6>> eigen_result(cov_pose);
    ...
    if (problem.Evaluate(eval_options, nullptr, nullptr, nullptr, &crs_J))
    {
        ...
        if (problem.Evaluate(eval_options, nullptr, &Residuals, nullptr, nullptr))
        {
...
#if 0
            Eigen::EigenSolver<Eigen::MatrixXd> eigen_result(JtJ);
#else

            //  LOG(INFO)  << "Hpp: \n"
            // << std::fixed
            // << marginalization_info->latestP_Information_matrix;
           
           Eigen::EigenSolver<Eigen::MatrixXd> eigen_result(marginalization_info->latestP_Information_matrix);
#endif

            double sum_lambda = eigen_result.eigenvalues()(0, 0).real() + eigen_result.eigenvalues()(1, 0).real() + eigen_result.eigenvalues()(2, 0).real() +
            ...
            //  for (int i = 0; i < POSE_DIM; i++) {
            //      optimization_eigen_values.push_back(system_lambdas(i));
            //  }
